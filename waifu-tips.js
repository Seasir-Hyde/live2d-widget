...
!(function () {
  ...
  class n {
    constructor(e) {
      const { modelListPath: t, modelsPath: o } = e;
      if (!o) throw "LocalModel requires modelListPath and modelsPath!";
      const s = o.endsWith("/") ? o : o + "/";
      (this.modelsPath = s), (this.modelListPath = t ?? s + "model_list.json");
    }
    async loadModelList() {
      try {
        if (
          ((this.modelList = await (async e => {
            const t = await fetch(e);
            if (!t.ok) throw new Error(`HTTP error! status: ${t.status}`);
            return await t.json();
          })(this.modelListPath)),
          console.log(this.modelListPath, this.modelList),
          !this.modelList.models || !this.modelList.messages)
        )
          throw "Invalid model list format!";
      } catch (e) {
        throw (console.error("Failed to load model list:", e), "Failed to load model list!");
      }
    }
    async loadModel(t, s, i) {
      localStorage.setItem("modelId", t),
        ...
        o(i, 4e3, 10),
        this.modelList || (await this.loadModelList());
      const n = e(this.modelList.models[t]);
      loadlive2d("live2d", `${this.modelsPath}${n}/index.json`), console.log(`Live2D 模型 ${n} 加载完成`);
    }
    async loadRandModel() {
      const e = localStorage.getItem("modelId");
      let t = localStorage.getItem("modelTexturesId");
      this.modelList || (await this.loadModelList());
      const s = this.modelList.models[e];
      if (Array.isArray(s)) {
        let i;
        do {
          i = Math.floor(Math.random() * s.length);
        } while (i === parseInt(t) && s.length > 1);
        if (i === parseInt(t)) return void o("我还没有其他衣服呢！", 4e3, 10);
        this.loadModel(e, i, "我的新衣服好看嘛？");
      } else o("我还没有其他衣服呢！", 4e3, 10);
    }
    async loadOtherModel() {
      let e = parseInt(localStorage.getItem("modelId"));
      this.modelList || (await this.loadModelList());
      const t = ++e >= this.modelList.models.length ? 0 : e;
      this.loadModel(t, 0, this.modelList.messages[t]);
    }
  ...
  function a(t) {
    const a = t.isLocalModel ? new n(t) : new s(t);
    function l(t) {
      let s,
        i,
        n = !1,
      ...
      window.addEventListener("mousemove", () => (n = !0)),
        ...
        window.addEventListener("mouseover", s => {
          for (let { selector: n, text: a } of t.mouseover)
            if (s.target.closest(n)) {
              if (i === n) return;
              return (i = n), (a = e(a)), (a = a.replace("{text}", s.target.innerText)), void o(a, 4e3, 8);
            }
        }),
        window.addEventListener("click", s => {
          for (let { selector: i, text: n } of t.click)
            if (s.target.closest(i))
              return (n = e(n)), (n = n.replace("{text}", s.target.innerText)), void o(n, 4e3, 8);
        }),
        t.seasons.forEach(({ date: t, text: o }) => {
          ...
          i.split("/")[0] <= s.getMonth() + 1 &&
            ...
            i.split("/")[1] <= s.getDate() &&
            s.getDate() <= n.split("/")[1] &&
            ((o = (o = e(o)).replace("{year}", s.getFullYear())), a.push(o));
        });
      const l = () => {};
      console.log("%c", l),
        (l.toString = () => {
    ...
    localStorage.removeItem("waifu-display"),
      ...
      (function () {
        (i["switch-model"].callback = () => a.loadOtherModel()),
          (i["switch-texture"].callback = () => a.loadRandModel()),
    ...
    o.addEventListener("click", () => {
      o.classList.remove("waifu-toggle-active"),
        o.getAttribute("first-time")
          ? (a(e), o.removeAttribute("first-time"))
          : (localStorage.removeItem("waifu-display"),
